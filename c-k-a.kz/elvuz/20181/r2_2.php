...
$myrow = mysql_fetch_array($result);
$i = 1;
echo "<a href=p2.php?ved=$ved&groups=$groups&kurs=$kurs>Печать</a><table border=1><tr><td>№</td><td align=center><b>Ф.И.О. слушатели</td><td><b>Номер зачетной книжки</td><td><b>Проценты</td><td><b>Баллы</td><td><b>Буквенная</td></tr>";
do
{ 
//r1
...
do
{
  echo "<a href=r2_2.php?ved=$myrow[ved]&groups=$groups&kurs=$kurs>$myrow[dis]</a><br><br>";
}
while ($myrow = mysql_fetch_array($result));
...
echo "<p>";
	do
		{
			  echo "<a href=r2_2.php?groups=$myrow[groupID]&sem=1&kurs=$myrow[kurs]>$myrow[name]</a> | ";
		}
	while ($myrow = mysql_fetch_array($result));
echo "</p>";
