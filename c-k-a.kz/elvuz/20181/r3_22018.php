...
$myrow = mysql_fetch_array($result);
$i = 1;
echo "<a href=p32018.php?ved=$ved&groups=$groups&kurs=$kurs>Печать</a><table border=1><tr><td>№</td><td align=center><b>Ф.И.О. слушатели</td><td><b>Номер зачетной книжки</td><td><b>Проценты</td><td><b>Баллы</td><td><b>Буквенная</td></tr>";
do
{ 
//r1
$result2 = mysql_query ("select * from journal20182 where StudentID=$myrow[StudentID] and StudyGroupID=$ved and markTypeID=7  and isCurrent=1");
...
$result = mysql_query ("select * from disved where groupID=$groups and god=2018 and sem=1 and kur=1");
$myrow = mysql_fetch_array($result);
do
{
  echo "<a href=r3_22018.php?ved=$myrow[ved]&groups=$groups&kurs=$kurs>$myrow[dis]</a><br><br>";
}
while ($myrow = mysql_fetch_array($result));
...
echo "<p>";
	do
		{
			  echo "<a href=r3_22018.php?groups=$myrow[groupID]&sem=2&kurs=$myrow[kurs]>$myrow[name]</a> | ";
		}
	while ($myrow = mysql_fetch_array($result));
echo "</p>";
