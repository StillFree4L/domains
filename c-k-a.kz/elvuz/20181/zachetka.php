...
//					$teacher=$_REQUEST['teacher' . $i];
		...
		$result = mysql_query("update students2018 set zachetka='$zach' where StudentID='$a'");  
...
$result1 = mysql_query("select * from groups2018 order by name");
$myrow1 = mysql_fetch_array($result1);

do
{
	echo "<h1>$myrow1[name]<h1>";
	$result = mysql_query("select * from students2018 where groupID=$myrow1[groupID] order by StudentID");
	...
	{
		$i++;
//		$result1 = mysql_query("UPDATE  `nitro`.`students2018` SET  `zachetka` =  '$myrow[zachetka]' WHERE  `students2018`.`StudentID` =$myrow[StudentID] ;"); 
