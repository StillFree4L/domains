...
$myrow = mysql_fetch_array($result);
$i = 1;
echo "<a href=p3.php?ved=$ved&groups=$groups&kurs=$kurs>Печать</a><table border=1><tr><td>№</td><td align=center><b>Ф.И.О. слушатели</td><td><b>Номер зачетной книжки</td><td><b>Проценты</td><td><b>Баллы</td><td><b>Буквенная</td></tr>";
do
{ 
//r1
$result2 = mysql_query ("select * from journal20182 where StudentID=$myrow[StudentID] and StudyGroupID=$ved and markTypeID=7");
...
do
{
  echo "<a href=r3_2.php?ved=$myrow[ved]&groups=$groups&kurs=$kurs>$myrow[dis]</a><br><br>";
}
while ($myrow = mysql_fetch_array($result));
...
echo "<p>";
	do
		{
			  echo "<a href=r3_2.php?groups=$myrow[groupID]&sem=1&kurs=$myrow[kurs]>$myrow[name]</a> | ";
		}
	while ($myrow = mysql_fetch_array($result));
echo "</p>";
