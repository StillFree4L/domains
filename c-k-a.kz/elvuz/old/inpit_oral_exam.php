...
echo "<tr><td class=le-content><a href=inpit_oral_exam.php?kurs=$myrow[kurs]&testID=$myrow[id]&type=$myrow[type]&groupID=$groups>$myrow[dis]</td><td class=le-content>$myrow[lang]</td><td class=le-content>$myrow[kurs]</td><td class=le-content>$myrow[semestr]</td>";
...
//					 echo "$i - $StudentID - $ball<br>";
					...
					$myrow1 = mysql_fetch_array($result1);
						if (($myrow1['ball']=='')  and ($ball<>''))
							{  $result5= mysql_query("INSERT INTO `nitro`.`oral_exam` (id,studentID,testID,groupID,ball,god) VALUES (' ' ,'$StudentID','$testID','$groupID','$ball' ,'2017');");	}
						else 
