...
if (isset($_REQUEST['groups'])) 
	{
		...
		$myrow = mysql_fetch_array($result);
			do
				{
					  echo "<p><a href=extot.php?ved=$myrow[ved]&groups=$groups&sem=$sem&t=$myrow[type]&kurs=$kurs>$myrow[dis]</a></p>";
				}
			while ($myrow = mysql_fetch_array($result));
	}
...
if (isset($_REQUEST['ved'])) 
	{
		...
		$myrow = mysql_fetch_array($result);
		$i = 1;
		echo "<a href=extot.php?ved=$ved&groups=$groups&t=$t&kurs=$kurs>Печать</a>Соту1 + Р1 + Соту2 + Р2=РД<table border=1><tr><td>№</td><td align=center><b>Ф.И.О. слушатели</td><td><b>Номер зачетной	книжки</td><td><b>Проценты</td><td><b>Баллы</td><td><b>Буквенная</td></tr>";
	do
		{ 
if ($kurs==1)
{
