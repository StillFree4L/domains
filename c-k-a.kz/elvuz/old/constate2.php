...
do
{
  echo "<a href=constate2.php?groupID=$myrow[groupID]&sem=2>$myrow[name]</a> | ";
}
...
if (isset($_REQUEST['groupID']))
{
	...
	$myrow = mysql_fetch_array($result);
echo "<br><br><form action=constate2.php method=get name=edit><table><tr><td></td><td align=center><b>Наименование дисциплины</td> <td  align=center><b>Кредит</td> <td  align=center><b>Преподаватель</td> <td  align=center><b>Год</td> <td  align=center><b>Семестр</td><td  align=center><b>Просмотр</td></tr>";
$i=1;
	do
		{
			echo "<tr><td>$i - <input type=hidden name=disID$i value=$myrow[id]></td>";
			echo "<td>                      <input type=text name=dis$i value='$myrow[dis]' size=50></td>";
			echo "<td align=center><input type=text name=credit$i value='$myrow[credit]'></td>";
			echo "<td>                     <input type=text name=teacher$i  value='$myrow[teacher]'></td>";
			echo "<td align=center>$myrow[god]</td>";
			echo "<td align=center>$myrow[sem]</td>";
			echo "<td><a href=constate2.php?ved=$myrow[ved]&groupID=$groupID>VIEW</a></td></tr>";
			$i++;
		}
	while ($myrow = mysql_fetch_array($result));
$i--;
...
//sotu1
if ($sotu1<>'')		{ $result = mysql_query("update journal2 set Mark='$sotu1'  where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=6 and number=1"); }
if ($sotu2<>'')		{ $result = mysql_query("update journal2 set Mark='$sotu2'  where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=6 and number=2"); }
if ($r1<>'')			{ $result = mysql_query("update journal2 set Mark='$r1'  where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=2 and number=1"); }
if ($r2<>'')			{ $result = mysql_query("update journal2 set Mark='$sotu1'  where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=2 and number=2"); }
...
if (isset($_REQUEST['ved']))
{
	...
	$myrow = mysql_fetch_array($result);
echo "<br><br><form action=constate2.php method=get name=ins><table><tr><td align=center ><b>Код слушателя</td><td align=center><b>ФИО слушателя</td><td align=center><b>Зачетка</td><td align=center><b>Соту 1</td><td align=center><b>Р1</td><td align=center><b>Соту 2</td><td align=center><b>Р2</td><td align=center><b>Рейтинг допуска</td></tr>";
$i=1;
	do
		{
			...
			$result3 = mysql_query ("select * from journal2 where StudyGroupID=$ved and StudentID=$myrow[StudentID] and markTypeID=6 and number=2");
			$myrow3 = mysql_fetch_array($result3);

			$result4 = mysql_query ("select * from journal2 where StudyGroupID=$ved and StudentID=$myrow[StudentID] and markTypeID=2 and number=2");
