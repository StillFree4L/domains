...
echo "<td align=right class=border-ram>$i.</td><td class=border-ram>$myrow[StudentID]  $myrow[lastname] $myrow[firstname] $myrow[patronymic] </td>";
...
$result5 = mysql_query ("select * from test_work where studentID=$myrow[StudentID] and testID=$myrow6[testID] ");
...
$result5 = mysql_query ("select * from oral_exam where studentID=$myrow[StudentID] and testID=$myrow6[testID] ");
$myrow5 = mysql_fetch_array($result5);
//if (isset($myrow5['ball']) and ($myrow5['ball']>=0)) {
$mark1=$myrow5['ball']; 
//}
...
$result7 = mysql_query ("select * from test_work where studentID=$myrow[StudentID] and testID=$myrow6[testID] ");
$myrow7 = mysql_fetch_array($result7);

//if (isset($myrow5['ball']) and ($myrow5['ball']>=0)) {
if (($myrow5['ball']<50) or ($myrow5['ball']=='')) { $mark1=0; } 
...
{	
			...
			if (($mark2>=0) and    ($mark2 <=49))   		{$mark2=""; 		$bukva2= ""; 		$ball2 = ""; $f++;}
...
//$res=mysql_query("update from totalmarks set exammark=$mark1 and ap_exammark=$mark1 and ratings=$mark and ap_ratings=$mark and rating=$mark and ap_rating=$mark and totalmark=$mark2 and ap_totalmark=$mark2 where studygroupID=$ved and studentID=$myrow[StudentID] ");
...
//update journal4 set Mark='$sotu1'  where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=6 and number=1
}
$res=mysql_query("UPDATE  `nitro`.`totalmarks2` SET  `exammark` =  '$mark1' ,`ap_exammark` =  '$mark1', ratings=$mark, ap_ratings=$mark, rating=$mark, ap_rating=$mark, totalmark=$mark2, ap_totalmark=$mark2 WHERE  `totalmarks2`.studygroupID=$ved and `totalmarks2`.studentID=$myrow[StudentID]");
if ($res==true) {echo "OK";} else {echo "NO";}
echo "</tr>";
$i++;
}
