...
<a href=indiv.php?add>Добавить слушателя</a> |  <a href=indiv.php?view>Просмотр слушателя</a>  |  <a href=indiv.php?vedo>Ведомасти</a> 
...
echo "<br><br><p>";
	do
		{
			  echo "<a href=indiv.php?groups=$myrow[groupID]&sem=2&kurs=$myrow[kurs]>$myrow[name]</a> | ";
		}
...
echo "<br><br>";
			do
				{
					  echo "<p><a href=indiv.php?group=$groups&sem=$sem&t=$myrow[type]&kurs=$kurs&studentID=$myrow[StudentID]>$myrow[lastname] $myrow[firstname] $myrow[patronymic] </a></p>";
				}
			while ($myrow = mysql_fetch_array($result));
	}
...
echo "<table>";
			do
				{
					  echo "<tr><td >$myrow[sem]</td><td><p><a href=indiv.php?ved=$myrow[ved]&group=$group&sem=$sem&t=$myrow[type]&kurs=$kurs&studentID=$studentID>$myrow[dis]</a> </td></tr>";
				}
			while ($myrow = mysql_fetch_array($result));
echo "</table>";
	}
...
if (isset($_REQUEST['ved'])) 
	{
		...
		$kurs=$_REQUEST['kurs']; 
		$studentID=$_REQUEST['studentID']; 
		$result = mysql_query ("INSERT INTO  `nitro`.`indiv_student` (`id` ,`kurs` ,`groupID` ,`studentID` ,`testID`)VALUES (NULL ,  '$kurs',  '$group',  '$studentID',  '$ved');");
if ($result==true) {echo "Данные успешно добавлены";} else {echo "Данные не добавлены";}
		//$myrow = mysql_fetch_array($result);
		//	do
		//		{
		//			  echo "<p><a href=indiv.php?ved=$myrow[ved]&group=$group&sem=$sem&t=$myrow[type]&kurs=$kurs&studentID=$studentID>$myrow[dis]</a></p>";
		//		}
		//	while ($myrow = mysql_fetch_array($result));
	}
...
{
$del=$_REQUEST['del'];
$result2 = mysql_query ("delete  from indiv_student where id=$del");
...
{
$result = mysql_query ("select * from indiv_student order by groupID");
$myrow = mysql_fetch_array($result);
echo "<table>";
	do
		{
...
$myrow2 = mysql_fetch_array($result2);
  ...
  echo "<tr><td>$myrow1[name]</td><td>$myrow2[lastname] $myrow2[firstname] $myrow2[patronymic]</td><td>$myrow[testID]</td><td><a href=indiv_ex.php?vedo=$myrow3[ved]&type=$myrow3[type]&kurs=$myrow[kurs]>$myrow3[dis]</td><td><a href=indiv.php?view=1&del=$myrow[id]>Удалить</a></td></tr>";
		}
	while ($myrow = mysql_fetch_array($result));
echo "</table>";
