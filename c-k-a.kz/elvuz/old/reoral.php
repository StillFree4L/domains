...
echo "<p>";
	do
		{
			  echo "<a href=reoral.php?groups=$myrow[groupID]&kurs=$myrow[kurs]&lang=$myrow[lang]>$myrow[name]</a> | ";
		}
...
echo "<tr><td class=le-content><a href=reoral.php?kurs=$myrow[kurs]&testID=$myrow[id]&type=$myrow[type]&groupID=$groups>$myrow[namedis]</td><td class=le-content>$myrow[lang]</td><td class=le-content>$myrow[kurs]</td><td class=le-content>$myrow[semestr]</td>";
...
$groupID=$_REQUEST['groupID'];
			...
			$result3 = mysql_query ("select * from test where id=$testID");
			$myrow3 = mysql_fetch_array($result3);
			echo "<h1>$myrow3[namedis]</h1> <a href=print_oral_exam.php?kurs=$kurs&testID=$testID&type=$type&groupID=$groupID>Печать</a>";

			$result1 = mysql_query ("select * from students2 where groupID=$groupID");
			$myrow1 = mysql_fetch_array($result1);
$i=0;
echo "<form action=reoral.php method=get name=ins><table border=0><tr><td>Ф.И.О.</td><td>Тестирования</td><td>Устный</td></tr>";
do
{
$i++;
$result2 = mysql_query ("select * from oral_exam where studentID='$myrow1[StudentID]' and testID='$testID' and groupID='$groupID'");
$myrow2 = mysql_fetch_array($result2);

$result3 = mysql_query ("select * from test_work where studentID='$myrow1[StudentID]' and testID='$testID' and groupID='$groupID'");
$myrow3 = mysql_fetch_array($result3);
...
//					 echo "$i - $StudentID - $ball<br>";
					$result1 = mysql_query ("select * from oral_exam where studentID='$StudentID' and testID='$testID' and groupID='$groupID'");
					$myrow1 = mysql_fetch_array($result1);
						if (($myrow1['ball']=='')  and ($ball<>''))
							{  $result5= mysql_query("INSERT INTO `nitro`.`oral_exam` (id,studentID,testID,groupID,ball) VALUES (' ' ,'$StudentID','$testID','$groupID','$ball' );");	}
						else 
							{ $result = mysql_query("update oral_exam set ball='$ball'  where studentID='$StudentID' and testID='$testID' and groupID='$groupID'"); 	}
