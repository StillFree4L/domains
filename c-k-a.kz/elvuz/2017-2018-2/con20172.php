...
do
{
  echo "<a href=con20172.php?groupID=$myrow[groupID]&kurs=$myrow[kurs]>$myrow[name]</a> | ";
}
...
$kurs=$_REQUEST['kurs'];
	...
	$myrow = mysql_fetch_array($result);
echo "<br><br><form action=con20172.php method=get name=edit><table><tr><td></td><td align=center><b>Наименование дисциплины</td> <td  align=center><b>Кредит</td> <td  align=center><b>Преподаватель</td> <td  align=center><b>Год</td> <td  align=center><b>Семестр</td><td  align=center><b>Просмотр</td></tr>";
$i=1;
	do
		{
			...
			echo "<td><a href=con20172.php?ved=$myrow[ved]&groupID=$groupID&kurs=$kurs>VIEW</a></td></tr>";
			$i++;
		}
	while ($myrow = mysql_fetch_array($result));
$i--;
			...
			for ($i=1; $i<=$kol; $i++)
				{
					...
					$r2=$_REQUEST['r2' . $i];
$kur=$_REQUEST['kur' . $i];
					 echo "$i - $StudentID - $sotu1 - $r1 - $sotu2 - $r2 - $kur<br>";
	...
	$result3 = mysql_query ("select * from journal174 where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=6 and number=2");
	$myrow3 = mysql_fetch_array($result3);

	$result4 = mysql_query ("select * from journal174 where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=2 and number=2");
	$myrow4 = mysql_fetch_array($result4);

	$result5 = mysql_query ("select * from journal174 where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=7");
...
if ($myrow3['Mark']=='') 
	{ $result5= mysql_query("INSERT INTO `nitro`.`journal174` (`StudentID`, `Mark`, `MarkDate`, `StudyGroupID`, `markTypeID`, `number`) VALUES ('$StudentID', '$sotu2', '2017-04-20', '$ved', '6', '2');");	}
else { $result = mysql_query("update journal174 set Mark='$sotu2'  where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=6 and number=2"); }

if ($myrow4['Mark']=='') 
	{ $result5= mysql_query("INSERT INTO `nitro`.`journal174` (`StudentID`, `Mark`, `MarkDate`, `StudyGroupID`, `markTypeID`, `number`) VALUES ('$StudentID', '$r2', '2017-04-20', '$ved', '2', '2');");	}
else 
{ $result = mysql_query("update journal174 set Mark='$r2'  where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=2 and number=2"); }


if ($myrow5['Mark']=='') 
	{ $result6= mysql_query("INSERT INTO `nitro`.`journal174` (`StudentID`, `Mark`, `MarkDate`, `StudyGroupID`, `markTypeID`, `number`) VALUES ('$StudentID', '$kur', '2017-04-20', '$ved', '7', '');");	}
else 
{ $result = mysql_query("update journal174 set Mark='$kur'  where StudentID='$StudentID' and StudyGroupID='$ved' and markTypeID=7"); }
...
if (isset($_REQUEST['ved']))
{
	...
	$myrow = mysql_fetch_array($result);
echo "<br><br><form action=con20172.php method=get name=ins><table><tr><td align=center ><b>Код слушателя</td><td align=center><b>ФИО слушателя</td><td align=center><b>Зачетка</td><td align=center><b>Соту 1</td><td align=center><b>Р1</td><td align=center><b>Соту 2</td><td align=center><b>Р2</td><td align=center><b>Курсовая</td><td align=center><b>Рейтинг допуска</td></tr>";
$i=1;
	do
		{
		...
		if  ($kurs==1){
			...
			$result3 = mysql_query ("select * from journal174 where StudyGroupID=$ved and StudentID=$myrow[StudentID] and markTypeID=6 and number=2");
			$myrow3 = mysql_fetch_array($result3);

			$result4 = mysql_query ("select * from journal174 where StudyGroupID=$ved and StudentID=$myrow[StudentID] and markTypeID=2 and number=2");
			$myrow4 = mysql_fetch_array($result4);
                        $result5 = mysql_query ("select * from journal174 where StudyGroupID=$ved and StudentID=$myrow[StudentID] and markTypeID=7");
			$myrow5 = mysql_fetch_array($result5);
...
echo "<td><input type=text size=3 name=kur$i value=$myrow5[Mark]></td>";
