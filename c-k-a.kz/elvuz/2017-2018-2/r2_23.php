...
echo "<p>";
	do
		{
			  echo "<a href=r2_23.php?groups=$myrow[groupID]&sem=2&kurs=$myrow[kurs]>$myrow[name]</a> | ";
		}
...
if (isset($_REQUEST['groups'])) 
	{
		...
		$myrow = mysql_fetch_array($result);
			do
				{
					  echo "<p><a href=r2_23.php?ved=$myrow[ved]&groups=$groups&sem=$sem&kurs=$kurs>$myrow[dis]</a></p>";
				}
			while ($myrow = mysql_fetch_array($result));
	}
...
$kurs=$_REQUEST['kurs'];
		...
		$myrow = mysql_fetch_array($result);
		$i = 1;
		echo "<a href=p23.php?ved=$ved&groups=$groups&kurs=$kurs>Печать</a><table border=1><tr><td>№</td><td align=center><b>Ф.И.О. слушатели</td><td><b>Номер зачетной	книжки</td><td><b>Проценты</td><td><b>Баллы</td><td><b>Буквенная</td></tr>";
	do
		{ 
			//satu1
