...
//r2
	...
	$result44 = mysql_query ("select * from journal175 where StudentID=$myrow[StudentID] and StudyGroupID=$ved and markTypeID=7 and isCurrent=1");
	$myrow44 = mysql_fetch_array($result44);


$mark = round(($myrow1['Mark']+$myrow2['Mark']+$myrow3['Mark']+$myrow4['Mark']+$myrow44['Mark'])/5);
...
if (($mark>=50) and ($mark1>=50)) {
$res=mysql_query("UPDATE  `nitro`.`totalmarks18` SET  `exammark` =  '$mark1' ,`ap_exammark` =  '$mark1', ratings=$mark, ap_ratings=$mark, rating=$mark, ap_rating=$mark, totalmark=$mark2, ap_totalmark=$mark2 WHERE  `totalmarks18`.studygroupID=$ved and `totalmarks18`.studentID=$myrow[StudentID]");}
echo "</tr>";
$i++;
}
