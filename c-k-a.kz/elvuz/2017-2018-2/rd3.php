...
echo "<p>";
	do
		{
			  echo "<a href=rd3.php?groups=$myrow[groupID]&sem=2&kurs=$myrow[kurs]>$myrow[name]</a> | ";
		}
...
if (isset($_REQUEST['groups'])) 
	{
		...
		$myrow = mysql_fetch_array($result);
			do
				{
					  echo "<p><a href=rd3.php?ved=$myrow[ved]&groups=$groups&sem=$sem&kurs=$kurs>$myrow[dis]</a></p>";
				}
			while ($myrow = mysql_fetch_array($result));
	}
...
if (isset($_REQUEST['ved'])) 
	{
		...
		$myrow = mysql_fetch_array($result);
		$i = 1;
		echo "<a href=pd23.php?ved=$ved&groups=$groups&kurs=$kurs>Печать</a><table border=1><tr><td>№</td><td align=center><b>Ф.И.О. слушатели</td><td><b>Номер зачетной	книжки</td><td><b>Рейтинг 1</td><td><b>Рейтинг 2</td><td><b>Рейтинг допуска</td><td><b>Баллы</td><td><b>Буквенная</td></tr>";
	do
		{ 
		//satu1
