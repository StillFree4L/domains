...
echo "<p>";
	do
		{
			  echo "<a href=rd.php?groups=$myrow[groupID]&sem=1&kurs=$myrow[kurs]>$myrow[name]</a> | ";
		}
...
if (isset($_REQUEST['groups'])) 
	{
		...
		$myrow = mysql_fetch_array($result);
			do
				{
					  echo "<p><a href=rd.php?ved=$myrow[ved]&groups=$groups&sem=$sem&kurs=$kurs>$myrow[dis]</a></p>";
				}
			while ($myrow = mysql_fetch_array($result));
	}
...
if (isset($_REQUEST['ved'])) 
	{
		...
		$myrow = mysql_fetch_array($result);
		$i = 1;
		echo "<a href=pd.php?ved=$ved&groups=$groups&kurs=$kurs>Печать</a><table border=1><tr><td>№</td><td align=center><b>Ф.И.О. слушатели</td><td><b>Номер зачетной	книжки</td><td><b>Рейтинг 1</td><td><b>Рейтинг 2</td><td><b>Рейтинг допуска</td><td><b>Баллы</td><td><b>Буквенная</td></tr>";
	do
		{ 
		//satu1
			$result1 = mysql_query ("select * from journal20181 where StudentID=$myrow[StudentID] and StudyGroupID=$ved and markTypeID=6 and number=1");
			$myrow1 = mysql_fetch_array($result1);
			//r1
			$result2 = mysql_query ("select * from journal20181 where StudentID=$myrow[StudentID] and StudyGroupID=$ved and markTypeID=2 and number=1");
			$myrow2 = mysql_fetch_array($result2);
			//satu2
			$result3 = mysql_query ("select * from journal20181 where StudentID=$myrow[StudentID] and StudyGroupID=$ved and markTypeID=6 and number=2");
			$myrow3 = mysql_fetch_array($result3);
			//r2
			$result4 = mysql_query ("select * from journal20181 where StudentID=$myrow[StudentID] and StudyGroupID=$ved and markTypeID=2 and number=2");
