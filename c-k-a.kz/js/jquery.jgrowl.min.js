...
/**
 * Show message in right top corner (by default)
 */
var Notify = {
    ...
    {
        $.jGrowl(data.text,{
            'sticky': parseInt(data.sticky) == 0 ? false : true,
            'theme': data.theme || 'ok',
            'life': data.life || 3000
        });
    },
    ...
    showMsg: function(data)
    {
        if(data && data.status && data.text){
            data.status = data.status == 'err' ? 'error' : data.status;
            var dt = {'sticky': data.sticky || 0, 'text': data.text, 'theme': data.status, 'life': data.life || 3000};
            this.show(dt);
        }
    }
...
/**
 * Check message return "ok" status
 *
 * @param data hash
 * @returns
 */
...
{
    return data && data.status && data.status == 'ok' ? true : false;
};