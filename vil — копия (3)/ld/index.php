...
if (trim($USER['wb_key']) != '')
{
   ...
   /* elseif ($_GET['type'] d== 3)
    ...
    if (($buf == "" || json_decode($buf2[1]) == NULL || time() - intval($buf2[0]) > 60*5 || strpos($buf, 'can\'t decode supplier key') !== false))
    {
        ...
        if ($api_url){
            if ($_GET['type'] == 1 or $_GET['type'] == 2){$r_url_report =  json_decode(report_cache());}
     ...
     //   $r_url =  json_decode(report_cache());
...
<a href='#' onclick="go_filtr(); return false;" class='btn' style='border: 1px solid #ccc; '>Фильтровать </a>
    ...
    if ($return_url){
        ?>
    <a href='<?=$return_url?>' class='btn' style='border: 1px solid #ccc; '><?=($config_return=='on' ? 'Откл' : 'Вкл')?> возвраты </a>
        <?php
    }
        }
    ?>
    ...
    foreach ($tbl_rows as $g)
    {
       // $g = (object) $g;
        if ($g->discountPercent==0 and $g->finishedPrice){
            $g->discountPercent = 100-floor(($g->finishedPrice*100)/$g->totalPrice);
         //   var_dump($g->totalPrice.' - '.$g->discountPercent.' - '.$g->finishedPrice);
        }
        if ($g->finishedPrice==0 and $g->discountPercent){
            $g->finishedPrice = $g->totalPrice-floor(($g->totalPrice*$g->discountPercent)/100);
          //  var_dump($g->totalPrice.' - '.$g->discountPercent.' - '.$g->finishedPrice);
        }
       ...
       // var_dump($g->doc_type_name);
        ...
        $g->totalPrice = number_format($g->totalPrice, 2, '.', '');
        $g->finishedPrice = number_format($g->finishedPrice, 2, '.', '');;
...
foreach ($tbl_keys as $k => $str) {
    ...
    if($_GET['type'] == 5 and !$_GET['rid']){
            if ($k == 'realizationreport_id' or $k == 'rr_dt' or $k == 'return_amount' or $k == 'delivery_amount'
                or $k == 'quantity' or $k == 'retail_amount' or $k == 'retail_commission' or $k == 'customer_reward'
                or $k == 'supplier_reward' or $k == 'storage_cost' or $k == 'acceptance_fee' or $k == 'other_deductions'
                or $k == 'for_pay' or $k == 'for_pay_nds' or $k == 'delivery_rub'){
                $column['hidden'] = false;
            }else{
                $column['hidden'] = true;
            }
...
foreach ($data as $d_k_k => $dat){
    ...
    foreach ($dat as $d_k => $da) {
        if (is_numeric($da) and ($d_k == 'totalPrice' or $d_k == 'finishedPrice' or $d_k == 'quantity' or $d_k == 'cost_amount'
                or $d_k == 'retail_price' or $d_k == 'retail_amount' or $d_k == 'retail_commission' or $d_k == 'sale_percent' or $d_k == 'commission_percent'
                or $d_k == 'customer_reward' or $d_k == 'supplier_reward' or $d_k == 'retail_price_withdisc_rub' or $d_k == 'for_pay'
                or $d_k == 'for_pay_nds' or $d_k == 'forPay' or $d_k == 'delivery_amount' or $d_k == 'return_amount' or $d_k == 'delivery_rub'
                or $d_k == 'product_discount_for_report' or $d_k == 'supplier_promo' or $d_k == 'supplier_spp')){
...
<script type = "text/javascript">
    ...
    if (document.referrer && document.referrer != null && document.referrer != ''){
        var doc_type = ((document.referrer).split("type=")[1]).split("&")[0];
        var doc_rid = (document.referrer).includes("rid");

        if (doc_type = <?=$_GET['type']?> && !isNaN(<?=$_GET['rid']?>) && !doc_rid){
            localStorage.clear();
        }
    }
    ...
    Ext.onReady(function() {
        ...
        store = Ext.create('Ext.data.Store', {
            ...
            proxy: {// описание прокси
                ...
                //url:'cache/data.json', // адрес серверноего обработчика
                reader: {
                    type: 'json',
                    rootProperty: 'data'
                },
