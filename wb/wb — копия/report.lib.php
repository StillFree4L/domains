...
function report_cache(){
    ...
    if (($lines[0] == "" or $lines[0] == null or json_decode($lines[1]) == NULL or intval($lines[0]) < $this_week) or (in_array($lines[1],[null,"[]","","can't decode supplier key","unauthorized","invalid token","supplier key not found"]))){
        $r = http($http);
