...
if(in_array($_GET['type'],[7,8]) and ($_GET['rid'] or $_GET['f1'])){
  $rid="";
  if($_GET['type']==7 and $_GET['rid']){$rid=' and incomeId="'.$_GET['rid'].'"';}
  if(in_array($_GET['type'],[7,8])){$type=7;}
  $result = mysqli_query($link, 'SELECT * FROM `values` WHERE `userId`='.$USER["id"].' and `type`='.$type.$rid);
  if ($result == false) {
...
if($data){
    ...
    foreach ($data as $d_k_k => $dat) {
        ...
        if(($_GET['type']==7 and $_GET['rid']) or ($_GET['type']==8 and $_GET['f1'])){
      ...
      }else if(($_GET['type']==7 and !$_GET['rid']) or ($_GET['type']==8 and !$_GET['f1'])){
                ...
                if ($dat->supplierArticle == $correct_line->supplierArticle and $dat->barcode == $correct_line->barcode) {
                    foreach ($correct_line as $k=>$item) {
                      if($k != 'barcode' and $k != 'supplierArticle'){
                        $data[$l]->$k = intval(number_format((string)$item, 0, '.', ''));
                      //  var_dump($data[$l]);
                      }
                    }
                }
