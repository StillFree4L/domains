...
function counts($rows){
    foreach ($rows as $key => $value) {
        ...
        elseif ($_GET['type'] == 2 and $value->totalPrice >= 0 and $value->isCancel != 1) {
...
$bars['date_length'] = '&ensp;'.(!$bool ? $bars['date'][0].' '.$days[date('w',strtotime($bars['date'][0]))].' - '.$bars['date'][count($bars['date'])-1].' '.$days[date('w',strtotime($bars['date'][count($bars['date'])-1]))] : date('d.m.Y', strtotime($_GET['dt'])));
$bars_old['date_length'] = '&ensp;'.(!$bool ? '&ensp;'.$bars_old['date'][0].' '.$days[date('w',strtotime($bars['date'][0]))].' - '.$bars_old['date'][count($bars['date'])-1].' '.$days[date('w',strtotime($bars['date'][0]))].' '.$days[date('w',strtotime($bars_old['date'][count($bars['date'])-1]))] : date('d.m.Y', strtotime('-1 DAY',strtotime($_GET['dt']))));
...
<div id="legend">
    ...
    <hr class="line" style="background-color:rgb(51, 102, 204);"/><span class="div">Текущий период: <?=$bars['date_length']?> &ensp; <?=$bars['finished']?> руб &ensp; <?=$bars["count"]?> шт</span>
    <hr class="line" style="background-color:red;"/><span class="div"> Предыдущий период: <?=$bars_old['date_length']?> &ensp; <?=$bars_old['finished']?> руб &ensp; <?=$bars_old["count"]?> шт</span>
    <span class="div">Разница: &ensp; <b><?=$difference['sum']?></b> руб &ensp; <b><?=$difference['count']?></b> шт</span>

    <div style="margin-top:-7px;" class="btn-group" data-toggle="buttons">
        <label class="btn btn-secondary" id="labeloption1">
            <input type="radio" name="options" id="option1" value="option1"> <span class="div1">Рубли</span>
        </label>
        <label class="btn btn-secondary" id="labeloption2">
            <input type="radio" name="options" id="option2" value="option2"> <span class="div1">Количество</span>
        </label>

    </div>

<button style="margin-top:-7px;" type="button" class="btn btn-secondary btn-sm" id="option3"  data-toggle="button" aria-pressed="false" autocomplete="off">
  Сглаживание
</button>
...
function drawChart() {
  ...
  var range_old = data_price.getColumnRange(3);

  if(range_old.max > range.max){
    range.max = range_old.max
  }
  if(range_old.min < range.min){
    range.min = range_old.min;
  }

  var re = /\B(?=(\d{3})+(?!\d))/g;

function tick_min(min,max){
    var min = 1;
...
function tick_max(min,max){
    if(max <= 10){
        ...
        var max = Math.ceil(max / 100) * 100;
    }
    else if(max <= 9999){
        var max = Math.ceil(max / 1000) * 1000;
    }
    else if(max <= 99999){
        var max = Math.ceil(max / 1000) * 1000;
    }
    else if(max <= 999999){
        var max = Math.ceil(range.max / 100000) * 100000;
    }
    else if(max > 999999){
        var max = Math.ceil(max / 100000) * 100000;
    }

    return max;
}

var max = tick_max(range.min,range.max);
var min = tick_min(range.min,max);
...
var data_quantity = new google.visualization.DataTable();
    ...
    var range_old = data_quantity.getColumnRange(3);
  ...
  if(range_old.min < range.min){
    ...
    var min = tick_min(range.min,range.max);
    var max = tick_max(range.min,range.max);
...
$option1 = "btn btn-secondary";
$option1_check = true;
$option2_check = false;
$option2 = "btn btn-secondary";
$option3 = "btn btn-secondary  btn-sm";
$curveType = '';
$st = "false";



if($status_checkbox->option == "option1"){
    $option1 .= " active";
    $ticks = "ticks_price";
    $data = "data_price";
    $option1_check = true;
    ...
    $option2 .= " active";
    $ticks = "ticks_quantity";
    $data = "data_quantity";
    $option2_check = true;
}
if($status_checkbox->status == "false"){
    $option3 .= " active";
    $curveType = 'function';
    $st = "true";
}
...
?>
    ...
    document.getElementById("labeloption1").className = '<?=$option1?>';
    document.getElementById("labeloption2").className = '<?=$option2?>';
    document.getElementById("option1").checked = '<?=$option1_check?>';
    document.getElementById("option2").checked = '<?=$option2_check?>';

    document.getElementById("option3").className = '<?=$option3?>';
    document.getElementById("option3").setAttribute('aria-pressed',<?=$st?>);
...
$('input[type=radio][name=options]').change(function() {
    ...
    $.get("/wb/update/update.php?option="+this.value, function (dt){});
...
document.getElementById("option3").onclick = function(){
  $.get("/wb/update/update.php?status="+this.getAttribute('aria-pressed'), function (dt){});
