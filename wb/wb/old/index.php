...
foreach ($tbl_keys as $k => $str) {
    ...
    if ($_GET['type'] == 5){
        ...
        if ('supplierArticle' == $k){
            $column['tpl'] = '<a href="?page=wb&type=8&f1={'.$k.'}&dt='.$_GET['dt'].'"><img height="20px" style="vertical-align: middle;" src="https://v1.iconsearch.ru/uploads/icons/gnomeicontheme/24x24/stock_list_bullet.png">{'.$k.'}</a>';
        }else {
    ...
    if ('storage_cost' == $k or $k == 'acceptance_fee' or $k == 'other_deductions' or ((($_GET['f1'] and $_GET['type']==8) or ($_GET['rid'] and $_GET['type']==7)) and (ru2Lat($k)=='Stoimosty_edinicy_tovara' or strpos(ru2Lat($ss_dop),ru2Lat($k))!==false))){
        if ($_GET['type']==5) {
           ...
           // $column['summaryType'] = 'sum';
            $column['tpl'] = "<input type=\"text\" idd='{id}' id='$k' incomeId='{incomeId}' barcode='{barcode}' supplierArticle='{supplierArticle}' class='inputValue' onblur=\"number_update('{id}',this.value,this.id,{incomeId},'{supplierArticle}',{barcode})\" onkeyup=\"this.value = this.value.replace(/[^^0-9\.]/g,'');\" value='{" . $k . "}'>";
            //echo '<pre>'; var_dump(json_encode($column));
        }
    ...
    if ($_GET['type']==8){
        if ($k == 'ss_one' or $k == 'ss_all' or $k == 'quantity' or $k == 'techSize' or ru2Lat($k)=='Stoimosty_edinicy_tovara' or strpos(ru2Lat($ss_dop),ru2Lat($k))!==false) {
            //$column['summaryType'] = 'sum';
        }
        if ($k == 'incomeId' and !$_GET['f1']) {
           // $column['summaryType'] = 'sum';
           $column['tpl'] = "<a href='?page=wb&type=" . $_GET['type'] . (isset($_GET['f1']) ? "&f1={$_GET['f1']}" : "") . "&f1={supplierArticle}&dt=".$_GET['dt']."'>{" . $k . "} шт</a>";
        }
    }
    ...
    foreach ($data as $d_k_k => $dat) {
        ...
        if ($_GET['type'] == 7 or $_GET['type'] == 8) {
            ...
            if ($_GET['rid'] or $_GET['f1']) {
                ...
                //echo '<pre>';var_dump($data[$l]->ss_all);
...
if ($_GET['type']==5) {
    ...
    function number_update(id,val,name,real,rid,barcode) {
        ...
        store.data.map[id].data[name] = val;
        store.data.map[id].data.ss_one = summinput.toFixed(2).replace(re," ");
        store.data.map[id].data.ss_all = (summinput*summkol).toFixed(2).replace(re," ");
        
        var b_sum = document.querySelector("b#"+name);
        var b_sum_ss_one = document.querySelector("b#ss_one");
        var b_sum_ss_all = document.querySelector("b#ss_all");          
        var inputs = document.querySelectorAll("input.inputValue#"+name);
        var sum_inp = document.querySelectorAll("div.inputSum");
        var sum_kol = document.querySelectorAll("div.inputSumKol");
            ...
            b_sum.innerHTML = sum.toFixed(2).replace(re," ");
            sum = 0;
            i = 0;
            ...
            b_sum_ss_one.innerHTML = sum.toFixed(2).replace(re," ");
            sum = 0;
            i = 0;
            ...
            b_sum_ss_all.innerHTML = sum.toFixed(2).replace(re," ");
