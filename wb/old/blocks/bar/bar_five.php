...
$finished = [];
$hideShow = false;
    //echo '<pre>'; var_dump($hide);
    //echo '<pre>'; var_dump($exceptions);
$i = 0;
foreach ($columnNames as $key=>$item){
   ...
   // echo '<pre>'; var_dump($tbl_keys[$item]);
...
//$lenghtDate = 'Период:&nbsp;'.date('d.m.Y',$columnArr['date'][count($columnArr['date'])-1]/1000).' - '.date('d.m.Y',$columnArr['date'][0]/1000).'&nbsp;&nbsp;&nbsp;'.$columnArr['price'][1]['name'].':&nbsp;'.$sumPrice1.' руб &nbsp;&nbsp;&nbsp;'.$columnArr['quantity'][0]['name'].':&nbsp;'.$sumPrice2.' шт';


   // echo '<pre>'; var_dump($columnArr['quantity'][0]);
        ...
        exporting: {
            buttons: {
                ...
                showHideButtons: {
                    ...
                    x: -260,
                    y: <?= $_GET['type'] == 9 ? -20 : -20?>,
                    text: 'Выделить/снять выделение',
                    theme: {
                        class: "button button2 highcharts-button highcharts-button-normal",
                    },
