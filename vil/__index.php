...
if (trim($USER['wb_key']) != '')
{
    ...
    {
        $sales_url = 'https://suppliers-stats.wildberries.ru/api/v1/supplier/sales?dateFrom=' . $dt1 . 'T00:00:00.000Z&key=' . $USER['wb_key'];
    ...
    {
        $url = 'https://suppliers-stats.wildberries.ru/api/v1/supplier/stocks?dateFrom=' . $dt1 . 'T00:00:00.000Z&key=' . $USER['wb_key'];
    ...
    {
        $orders_url = 'https://suppliers-stats.wildberries.ru/api/v1/supplier/orders?dateFrom=' . $dt1 . 'T00:00:00.000Z&flag=0&key=' . $USER['wb_key'];
        $api_url = $orders_url;

        $tbl_keys = ($orders_keys);
    }
    //$buf = file_get_contents('wb-cache/'.$USER['wb_key'].'-'.$_GET['type'].'-'.$_GET['dt']);
    $buf = file_get_contents('wb-cache/' . $USER['wb_key'] . '-' . $_GET['type']);
    $buf2 = explode('@@---@@', $buf);
    if ($buf == "" || json_decode($buf2[1]) == NULL || time() - intval($buf2[0]) > 5 * 60 || strpos($buf, 'can\'t decode supplier key') !== false)
    {
        ...
        {
            file_put_contents('wb-cache/' . $USER['wb_key'] . '-' . $_GET['type'], time() . '@@---@@' . $r);
        }
        else
        {
            $buf = explode('@@---@@', $buf);
...
$tps_res = [2 => 'Заказы', 1 => 'Продажи', 3 => 'Отмененные заказы', 4 => 'Отмененные продажи', 5 => 'Отчеты по реализации', 6 => 'Склад', 7 => 'Поставки', 8 => 'Себестоимость', 9 => 'Чистая прибыль'];
...
if ($_GET['type'] == 6)
{
    ...
    $buf = file_get_contents('wb-cache/' . $USER['wb_key'] . '-1');
    $buf = explode('@@---@@', $buf);
    $r = $r0 = $buf[1];

    $sales_rows = json_decode($r);

    $buf = file_get_contents('wb-cache/' . $USER['wb_key'] . '-2');
    $buf = explode('@@---@@', $buf);
    $r = $r0 = $buf[1];
