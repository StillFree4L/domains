...
$USER['id'] = 2;
$USER['wb_key'] = 'NGQ0M2ZiMmYtMmZmYS00NGUzLWE5ODktMzIxNThmMzY3NTkw';
...
if (trim($USER['wb_key']) != '')
{
    ...
    if ($_GET['type'] == 1 || $_GET['type'] == 4)
    {
        ...
        $sales_url = 'https://suppliers-api.wildberries.ru/api/v2/orders?date_start='.$dt1.'T00:00:00.000Z&take=1000&skip=0';
    ...
    {
        $report_url = 'https://suppliers-stats.wildberries.ru/api/v1/supplier/reportDetailByPeriod?dateFrom=' . $dt1 . '&key=' . $USER['wb_key'] . '&limit=1000000&rrdid=0&dateto=' . date('Y-m-d');
    ...
    {
        $url = 'https://suppliers-stats.wildberries.ru/api/v1/supplier/incomes?dateFrom=' . $dt1 . 'T00:00:00.000Z&key=' . $USER['wb_key'];
        $api_url = $url;

        //echo $api_url;echo "<hr>".http($api_url);
    ...
    {
        $orders_url = 'https://suppliers-api.wildberries.ru/api/v2/orders?date_start='.$dt1.'T00:00:00.000Z&take=1000&skip=0';
    ...
    //$buf = file_get_contents('wb-cache-new/'.$USER['wb_key'].'-'.$_GET['type'].'-'.$_GET['dt']);
    $buf = file_get_contents('wb-cache-new/' . $USER['wb_key'] . '-' . $_GET['type']);
    $buf2 = explode('@@---@@', $buf);
    if ($buf == "" || json_decode($buf2[1]) == NULL || time() - intval($buf2[0]) > 5 * 60 || strpos($buf, 'can\'t decode supplier key') !== false)
    {
        ...
        {
            file_put_contents('wb-cache-new/' . $USER['wb_key'] . '-' . $_GET['type'], time() . '@@---@@' . $r);
        }
        else
        {
            $buf = explode('@@---@@', $buf);
    ...
    if($_GET['type'] == 2 || $_GET['type'] == 10){
      /* $sk_url = 'https://suppliers-api.wildberries.ru/api/v2/stocks?skip=0&take=8000';
        $ra = http($sk_url);
        sleep(2);
        $ra = json_decode($ra);*/
        $r = orders_object($r,$_GET['type']);

    }elseif($_GET['type'] == 1){
        $r = sales_object($r);
    }elseif($_GET['type'] == 6){
        $r = stocks_object($r,$dt1);
    }

   //var_dump($r);

    $tbl_rows = $orig_tbl_rows = $r;
    if (strpos($r0, 'can\'t decode supplier key') !== false)
    {
        $bad_key = 1;
    }
...
';
    ...
    foreach ($tbl_rows as $g)
    {
        ...
        $g->incomeId = '<a href="./index_new.php?page=wb&type=7&rid=' . $g->incomeId . '&dt=' . $_GET['dt'] . '"><img height="20px" style="vertical-align: middle;" src="https://v1.iconsearch.ru/uploads/icons/gnomeicontheme/24x24/stock_list_bullet.png">' . $g->incomeId . '</a>';
...
'));
       ...
       // var_dump($g);
        ...
        $g->barcode = '<a href="./index_new.php?page=wb&type=6&f1=' . $g->barcode . '&dt=' . $_GET['dt'] . '"><img height="20px" style="vertical-align: middle;" src="https://v1.iconsearch.ru/uploads/icons/gnomeicontheme/24x24/stock_list_bullet.png">' . $g->barcode . '</a>';
...
if ($_GET['type'] == 2 || $_GET['type'] == 3)
{
    ...
    foreach ($tbl_rows as $g)
    {
        ...
        if (!$g->isCancel && $_GET['type'] == 3) continue;
...
// '));
        ...
        $g->barcode = '<a href="./index_new.php?page=wb&type=' . $_GET['type'] . '&f1=' . $g->barcode . '&dt=' . $_GET['dt'] . $dop_dts_range . '"><img height="20px" style="vertical-align: middle;" src="https://v1.iconsearch.ru/uploads/icons/gnomeicontheme/24x24/stock_list_bullet.png">' . $g->barcode . '</a>';
...
if ($_GET['type'] == 1 || $_GET['type'] == 4) {
    ...
    foreach ($tbl_rows as $g)
    {
        if ($g->totalPrice > 0 && $_GET['type'] == 4) continue;
...
'));
        ...
        if (!isset($_GET['f1'])) $g->supplierArticle = '<a href="./index_new.php?page=wb&type=8&f1=' . $g->supplierArticle . '&dt=' . $_GET['dt'] . '"><img height="20px" style="vertical-align: middle;" src="https://v1.iconsearch.ru/uploads/icons/gnomeicontheme/24x24/stock_list_bullet.png">' . $g->supplierArticle . '</a>';
...
quantity'));
        ...
        $g->barcode2 = $g->barcode;
        //$g->barcode  = '<a href="./index_new.php?page=wb&type=9&rid='.$g->realizationreport_id.'&bc='.$g->barcode.'"><img height="20px" style="vertical-align: middle;" src="https://v1.iconsearch.ru/uploads/icons/gnomeicontheme/24x24/stock_list_bullet.png">'.$g->barcode .'</a>';
        $g->realizationreport_id2 = $g->realizationreport_id;
        $g->realizationreport_id = '<a href="./index_new.php?page=wb&type=9&rid=' . $g->realizationreport_id . '"><img height="20px" style="vertical-align: middle;" src="https://v1.iconsearch.ru/uploads/icons/gnomeicontheme/24x24/stock_list_bullet.png">' . $g->realizationreport_id . '</a>';
