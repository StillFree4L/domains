...
function stock_cache_old($load=null){
    $dir = 'cache/stocks';
...
//кэшир склад new api
function stock_cache_new($load=null){
    $dir = 'cache/stocks';
...
function api_valid($valid){
  if($valid and in_array($valid,[null,'',' '])){
    return '<font color="#ff7f50">нет ответа от сервера api, повторите проверку позднее</font>';
  }else if($valid and (in_array($valid,["invalid token",'supplier key not found','unauthorized','can\'t decode supplier key']) or ($valid->errors and in_array($valid->errors[0],["invalid token",'supplier key not found','unauthorized','can\'t decode supplier key'])))){
...
function getResponseByUrlsMulti($urls,$json=null, $followLocation = false, $maxRedirects = 10)
{
    // Options
    $curlOptions = [
        CURLOPT_NOBODY => false,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_HTTPHEADER => array('Content-Type: application/json','Authorization: Bearer '.$GLOBALS["auth"])
    ];
